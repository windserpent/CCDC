...
# All Security Updates Script for Fedora 42
...
# Valid switchs are:
# ./get_all_security_updates_f42.sh preview
# ./get_all_security_updates_f42.sh install

set -euo pipefail

# Configuration
LOG_FILE="/var/log/all_security_updates_f42.log"
...
get_high_impact_cves() {
    ...
    # Look for specific high-impact CVE patterns - expanded to include Moderate
    dnf updateinfo list security 2>/dev/null | grep -iE "(CVE-[0-9]{4}-[0-9]{4,})" | \
    grep -E "(Critical|Important|Moderate)" | while read -r line; do
        advisory=$(echo "$line" | awk '{print $1}')
        severity=$(echo "$line" | awk '{print $2}')
        package=$(echo "$line" | awk '{print $3}')
        
        # Only include packages from our critical list
        if echo "$package" | grep -qE "^(kernel|sudo|glibc|openssl|systemd|openssh|pam|polkit|bind|unbound|podman|buildah|skopeo|runc|crun|dnf|rpm|selinux-policy|firefox|mesa|xorg-x11|wayland)"; then
...
preview_updates() {
    ...
    if [[ -n "$security_updates" ]]; then
        ...
        if [[ -n "$critical_updates" ]]; then
            echo -e "${RED}Critical security updates for essential packages:${NC}"
        ...
        if [[ -n "$important_updates" ]]; then
            echo -e "${YELLOW}Important security updates for essential packages:${NC}"
        ...
        if [[ -n "$moderate_updates" ]]; then
            echo -e "${BLUE}Moderate security updates for essential packages:${NC}"
    ...
    if [[ -n "$cve_updates" ]]; then
        echo -e "${YELLOW}CVE-based security updates:${NC}"
        echo "$cve_updates" | while read -r advisory severity package cve; do
            if [[ -n "$cve" ]]; then
                case "$severity" in
                    "Critical") echo -e "  ${RED}$severity${NC}: $package ($advisory - $cve)" ;;
                    "Important") echo -e "  ${YELLOW}$severity${NC}: $package ($advisory - $cve)" ;;
                    "Moderate") echo -e "  ${BLUE}$severity${NC}: $package ($advisory - $cve)" ;;
                esac
            else
                case "$severity" in
                    "Critical") echo -e "  ${RED}$severity${NC}: $package ($advisory)" ;;
                    "Important") echo -e "  ${YELLOW}$severity${NC}: $package ($advisory)" ;;
                    "Moderate") echo -e "  ${BLUE}$severity${NC}: $package ($advisory)" ;;
                esac
            fi
        done
        echo
    ...
    packages_list=$(
        {
            echo "$security_updates" | awk '{print $3}'
...
install_updates() {
    echo -e "${BLUE}=== INSTALLING FEDORA 42 ALL SECURITY UPDATES ===${NC}"
    ...
    {
        echo "$security_updates" | awk '{print $2}'
        echo "$cve_updates" | awk '{print $2}'
    ...
    # Check for desktop/graphics updates
    if echo "$packages_list" | grep -qE "(mesa|xorg-x11|wayland)"; then
        echo -e "${YELLOW}Graphics stack updated - consider restarting desktop session${NC}"
        log "Graphics stack updated"
    fi
...
# Main function
main() {
    echo -e "${BLUE}Fedora 42 All Security Updates${NC}"
    echo "=============================="
